// ==========================================
// UNDEAD BARRAGE V2.0.0 - GLOBAL CONFIGURATION
// Plain global: window.GameConfig (no ES modules)
// All balance knobs live here. No gameplay numbers in game.js.
// ==========================================
window.GameConfig = {
    ...
    director: {
        ...
        crateFallSpeed: 60       // crate type is a 50/50 health/grenade split (see SupplyCrate)
    },
    ...
    player: {
        playerScale: 2.0,
        baseSpeed: 180,
        dashSpeedMult: 2.5,      // Reserved — not implemented in v1
        dashDuration: 250,       // Reserved — not implemented in v1
        dashCooldown: 1500,      // Reserved — not implemented in v1
        coinLifespan: 10000,
        coinDrag: 0.95,
        coinBounce: 0.5,
        magnetRadius: 150
    ...
    system: {
        masterVolume: 1.0,
        sfxVolume: 1.0,
        musicVolume: 0.5,        // Reserved — no music track in v1
        showHitboxes: false,
        audioEnabled: true       // master kill switch for audio
    },

    // ARENA GEOMETRY (road layout + fixed Y offsets from bottom of screen)
    arena: {
        roadWidth: 500,           // px, centered horizontally
        enemySpawnY: -40,         // px above top edge
        playerStartOffsetY: 120,  // px from bottom of screen
        barricadeY: -220,         // offset from bottom: cam.height + this
        turretY: -200,            // offset from bottom
        crateRestY: -260,         // offset from bottom
        mobScale: 1.5,            // display scale for standard mobs (boss uses bosses[].scale)
        edgeMargin: 30            // spawn/clamp inset from road edges
    },

    // HUD DISPLAY
    hud: {
        transmissionAutoHideMs: 8000,   // ms before non-gate transmissions hide
        waveClearFlashMs: 1300,         // ms for wave cleared message
        comboDisplayMin: 2              // minimum combo before showing counter
    },
    ...
    defenses: {
        barricade: { hp: 100, slots: 6, repairFraction: 0.25 },
        turret:    { range: 400, rate: 300, damage: 30, projectileSpeed: 1500 }
    },

    // 12. COMBAT TIMINGS & MISC
    // grenadeDamage/grenadeRadius decouple the thrown frag from the Grenade Launcher weapon.
    combat: {
        hitInvulnMs: 800,
        grenadeFuseMs: 700,
        grenadeSpeed: 500,
        ...
        mobCoinMin: 1,
        mobCoinMax: 3,
        pierceMaxHits: 3,        // pierce projectiles pass through up to this many enemies
        knockbackFactor: 0.35,   // px push per knockback point on hit
        bossKnockbackFactor: 0.08
    }
};
