/**
 * TONY KIOSK ENGINE // V0.3.1
 * Core Logic: Reduced Offseason footprint & Record detection
 */
...
const WeatherPipeline = {
    ...
    async sync() {
        ...
        const current = timelines.minutely?.[0]?.values || timelines.hourly?.[0]?.values;
...
const SportsPipeline = {
    ...
    renderRow(team, data) {
        ...
        if (events.length === 0) return `<div class="h-8 flex items-center px-4 bg-slate-900/20 rounded-lg text-[10px] font-mono text-slate-600 uppercase tracking-widest">${team.name} // HIBERNATION</div>`;
        ...
        const myTeam = comp.competitors.find(c => c.id === team.id);
        const opp = comp.competitors.find(c => c.id !== team.id);

        // Detect Record
        const record = myTeam.record?.[0]?.summary || myTeam.records?.[0]?.summary || "0-0";
        const lastResult = past[0] ? (past[0].competitions[0].competitors.find(c => c.id === team.id).winner ? 'W' : 'L') : '';

        if (target.status.type.state === 'post' && (now - new Date(target.date)) > 864000000) {
             return `<div class="h-8 flex justify-between items-center px-4 bg-slate-900/20 rounded-lg text-[10px] font-mono text-slate-600 uppercase tracking-widest"><span>${team.name}</span><span>OFFSEASON // ${record}</span></div>`;
        }

        const isLive = target.status.type.state === 'in';
        return `
            <div class="${isLive ? 'bg-slate-800/80 border-slate-500' : 'bg-slate-900/40 border-slate-800'} border rounded-2xl p-4 flex justify-between items-center">
                <div class="flex items-center gap-3">
                    <img src="${myTeam.team.logos?.[0]?.href}" class="w-8 h-8 object-contain">
                    <div>
                        <div class="font-bold text-sm">${team.name} <span class="text-[10px] font-mono text-slate-500 ml-1">${record}</span></div>
                        <div class="text-[10px] text-slate-400">vs ${opp.team.abbreviation} ${lastResult ? `<span class="ml-2 text-slate-600">PREV: ${lastResult}</span>` : ''}</div>
                    </div>
                </div>
                <div class="text-right font-mono">
                    <div class="text-sm ${isLive ? 'text-red-500 animate-pulse' : 'text-white'}">${isLive ? `${myTeam.score}-${opp.score}` : new Date(target.date).toLocaleTimeString([], {hour:'numeric', minute:'2-digit'})}</div>
                    <div class="text-[9px] text-slate-500 uppercase">${isLive ? target.status.type.shortDetail : new Date(target.date).toLocaleDateString([], {month:'short', day:'numeric'})}</div>
                </div>
            </div>`;
    },
